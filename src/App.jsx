...
function App() {
  ...
  // 步驟 2: 張數選擇
  const [count, setCount] = useLocalStorage('count', 8)

  // 步驟 3: 角色描述/圖片和主題說明
  const [characterDescription, setCharacterDescription] = useLocalStorage('characterDescription', '')
  const [theme, setTheme] = useLocalStorage('theme', '')
  const [uploadedCharacterImage, setUploadedCharacterImage] = useLocalStorage('uploadedCharacterImage', null)

  // 步驟 4: 角色生成/確認
  const [characterImage, setCharacterImage] = useLocalStorage('characterImage', null)
  const [characterConfirmed, setCharacterConfirmed] = useLocalStorage('characterConfirmed', false)
  const [generatingCharacter, setGeneratingCharacter] = useState(false)

  // 步驟 5: 文字風格描述
  const [textStyle, setTextStyle] = useLocalStorage('textStyle', '')
  const [generatingTextStyle, setGeneratingTextStyle] = useState(false)
  const [textStyleConfirmed, setTextStyleConfirmed] = useLocalStorage('textStyleConfirmed', false)

  // 步驟 6: 文字描述
  const [descriptions, setDescriptions] = useLocalStorage('descriptions', [])
  const [generatingDescriptions, setGeneratingDescriptions] = useState(false)
  const [excludedTexts, setExcludedTexts] = useLocalStorage('excludedTexts', '') // 排除的文字（每行一個）
  const [characterStance, setCharacterStance] = useLocalStorage('characterStance', '') // 角色立場描述（可選）

  // 步驟 6-8: 8宮格生成、去背、裁切
  const [gridImages, setGridImages] = useLocalStorage('gridImages', []) // 8宮格圖片陣列
  const [processedGridImages, setProcessedGridImages] = useLocalStorage('processedGridImages', []) // 去背後的8宮格
  const [cutImages, setCutImages] = useLocalStorage('cutImages', []) // 裁切後的單張圖片
  const [mainImage, setMainImage] = useLocalStorage('mainImage', null) // 主要圖片 240x240
  const [tabImage, setTabImage] = useLocalStorage('tabImage', null) // 標籤圖片 96x74
  const [backgroundThreshold, setBackgroundThreshold] = useLocalStorage('backgroundThreshold', 240) // 去背閾值
    ...
    return (
      <div className="app" style={{
        ...
        <div className="container" style={{
          ...
          <h1 className="title" style={{ textAlign: 'center', marginBottom: '30px', color: '#333' }}>
            🔒 專案登錄
          </h1>
          <form onSubmit={handleLogin}>
            ...
            {loginError && (
              <p style={{ color: '#ff4d4f', marginBottom: '15px', fontSize: '14px', textAlign: 'center' }}>
                {loginError}
              </p>
            )}
            <button
  ...
  // 重置專案
  const handleReset = () => {
    if (window.confirm('確定要重置專案嗎？所有生成的內容都會消失。\n(API Key 會保留)')) {
      setCount(8)
    ...
    <div className="app">
      <div className="container">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <h1 className="title" style={{ margin: 0 }}>LINE 貼圖製作</h1>
          {currentStep > 1 && (
            <button
              onClick={handleReset}
              className="btn btn-secondary"
              style={{ padding: '8px 12px', fontSize: '14px', background: '#f44336', color: 'white', border: 'none' }}
            >
              重置專案
            </button>
          )}
